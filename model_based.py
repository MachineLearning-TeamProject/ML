...
#
#     def save_array(self):
#         np.save('parameter/bias', self._b)
#         np.save('parameter/bias_user', self._b_P)
#         np.save('parameter/bias_item', self._b_Q)
#         np.save('parameter/user_latent', self._P)
#         np.save('parameter/item_latent', self._Q)
#
#     def load_array(self):
#         self._b = np.load('parameter/bias.npy')
#
#         bias_user = np.load('parameter/bias_user.npy')
#         self._b_P[:bias_user.shape[0]] = bias_user
#
#         bias_item = np.load('parameter/bias_item.npy')
#         self._b_Q[:bias_item.shape[0]] = bias_item
#
#         user_latent = np.load('parameter/user_latent.npy')
#         self._P[:user_latent.shape[0], :] = user_latent
#
#         item_latent = np.load('parameter/item_latent.npy')
#         self._Q[:item_latent.shape[0], :] = item_latent
#
...
class MatrixFactorization():
    ...
    def test(self, user_id):
        predicted = self.get_complete_matrix()
        predicted = np.round(predicted, 2)
        predicted = np.where(predicted > 16.5, predicted, 0.0)
        result = pd.DataFrame(predicted, index=self._index, columns=self._columns).T[user_id]
        return result

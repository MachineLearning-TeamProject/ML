...
# 사용자 특성 벡터 생성 함수
def create_user_vector(user_preferences, features):
    vectorizer = CountVectorizer()
    user_vector = vectorizer.fit_transform([user_preferences])
    features_vector = vectorizer.transform(features)
    return user_vector, features_vector

# 사용자 선호도를 입력하여 특정 사용자에게 추천할 여행지를 찾는 함수
def recommend_destinations(user_preferences, data, num_recommendations=10):
    features = data.columns[1:]  # 여행지의 특성들
    user_vector, features_vector = create_user_vector(user_preferences, features)

    # 코사인 유사도 계산
    similarity_scores = cosine_similarity(user_vector, features_vector)

    # 가장 유사한 여행지들의 인덱스를 추출
    recommended_destination_indices = similarity_scores.argsort()[0][-num_recommendations:][::-1]

    # 추천 여행지 리스트
    recommended_destinations = data.iloc[recommended_destination_indices]['VISIT_ID'].tolist()
...
if __name__ == "__main__":
    ...
    # 예시: 특정 사용자의 선호도
    user_preferences = "테마파크, 놀이시설, 동/식물원 방문, 야외 스포츠, 레포츠 활동, 지역 문화예술/공연/전시시설 관람"
    recommended_destinations = recommend_destinations(user_preferences, merged_table)
    print(f"추천 여행지: {recommended_destinations}")